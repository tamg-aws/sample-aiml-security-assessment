...
A serverless framework that scans AWS accounts for AI/ML security misconfigurations and produces interactive HTML reports. The full catalog contains 218 checks across seven assessment areas: 104 core checks (46 Amazon Bedrock, 33 Amazon SageMaker AI with `SM-29` reserved, 17 Amazon Bedrock AgentCore, and 8 AWS Agent Registry), 38 always-on Agentic AI Security checks, 64 optional Responsible AI GRC checks, and 12 optional OWASP Top 10 for LLM checks. Checks are derived from the AWS Well-Architected Generative AI Lens, the Agentic AI Lens, AWS Responsible AI GRC guidance, and the OWASP Top 10 for LLM 2025.
...
- **Check ID prefixes** drive report routing: `BR-` Bedrock, `SM-` SageMaker, `AC-` AgentCore, `AR-` AWS Agent Registry, `AG-` Agentic AI, `FS-` Responsible AI GRC (legacy identifier retained for compatibility), `OW-` OWASP Top 10 for LLM, and `AISF-` AWS AI Security Framework. The report reconstructs Agentic AI from `AG-` and compliance sections through `COMPLIANCE_STANDARDS` in `report_template.py`. An unrecognised prefix does not error: the multi-account consolidator falls through to `service = "bedrock"`, so a new prefix is misfiled silently until it is registered.
- **AISF is a derived compliance standard.** No Lambda, no CSV, no state-machine branch: `derive_aisf_findings()` in `generate_consolidated_report/aisf_mappings.py` restates existing check verdicts as `AISF-` rows at consolidation time, in both the report Lambda and root `consolidate_html_reports.py`. Its `COMPLIANCE_STANDARDS` entry carries `"derived": True`, which keeps the slug out of the S3 prefix list `app.py` builds; a derived slug reaching that list is an `AccessDenied` from the report Lambda's own `s3:ListBucket` condition and fails report generation for every category. `AISF-` rows are excluded from the 218-check total because they carry no new assertion. Only controls with ledger verdict `covered` may be mapped, and `aisf-parity/check_ledger.py` gates the map against `aisf-parity/aisf-work-ledger.json` and the AISF control YAML.
...
6. **OWASP/compliance mapping drift** — every `OWASP_CHECK_MAPPINGS` source ID still exists in its scanner, every source CSV the OWASP Lambda reads actually feeds at least one mapping, every emitted `OW-` ID is documented in `docs/SECURITY_CHECKS_OWASP.md`, and `COMPLIANCE_STANDARDS` prefixes in `report_template.py` match the report/consolidator routing expectations. Values in mappings are lists (one source check can emit multiple OW rows). An `N/A` source must produce `Severity=Informational` on the OW row (never inherit High/Medium). For AISF: every control in `AISF_DERIVED_MAP` still has ledger verdict `covered` with the same incumbents, every emitted `AISF-` id is documented in `docs/SECURITY_CHECKS_AISF.md`, the baked `severity`/`resolution`/`reference` still match the AISF control YAML, and a derived standard still contributes no S3 prefix. `python3 aisf-parity/check_ledger.py` gates all of that and must report 13 of 13. Unlike OWASP, an `AISF-` row takes its severity from the AISF control's `risk` band and not from the source row, because one incumbent's severity is not the control's risk rating.
