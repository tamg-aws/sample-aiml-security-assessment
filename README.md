...
Run **[249 checks](docs/SECURITY_CHECKS.md)** across AWS accounts and regions:

- **135 always-on core checks** for Amazon Bedrock, Amazon SageMaker AI, Amazon Bedrock AgentCore, and AWS Agent Registry
...
- **[249 Security Checks](docs/SECURITY_CHECKS.md)** across Amazon Bedrock, Amazon SageMaker AI, Amazon Bedrock AgentCore, AWS Agent Registry, Agentic AI Security, Responsible AI GRC, and OWASP Top 10 for LLM
...
| **Inconsistent security checks across teams** | Standardized 249-check assessment based on AWS Well-Architected Generative AI Lens and Agentic AI Lens best practices, AWS Responsible AI governance/risk/compliance guidance, and OWASP Top 10 for LLM |
...
- **[Amazon Bedrock](docs/SECURITY_CHECKS.md#amazon-bedrock-security-checks-46)** (46 always-on core checks) - Covers guardrails, prompt-attack and image filtering, cross-account policies, data retention, inference profiles, automated reasoning and Marketplace endpoint encryption/networking, Amazon VPC endpoints, IAM permissions, agent guardrails and least privilege, logging, monitoring, evaluation, quotas, Lambda code scanning, central guardrail enforcement, model invocation allow-lists, Region and Marketplace subscription control, API key governance, and knowledge base source data classification.
- **[Amazon SageMaker AI](docs/SECURITY_CHECKS.md#amazon-sagemaker-ai-security-checks-33)** (33 always-on core checks) - Covers AWS Security Hub controls, internet and VPC exposure, encryption, isolation, GuardDuty AI Protection, HyperPod, Model Registry resource policies, MLOps, monitoring, approval, drift detection, deployment patterns, lineage tracking, endpoint inference data capture, AWS Config compliance evaluation, training job network boundary, and creation guardrails. `SM-29` remains reserved for a deferred Unified Studio networking check; `SM-30` through `SM-34` are implemented.
- **[Amazon Bedrock AgentCore](docs/SECURITY_CHECKS.md#amazon-bedrock-agentcore-security-checks-47)** (47 always-on core checks) - Covers runtime, Code Interpreter, and browser VPC isolation; Identity token-vault encryption; browser recording; memory, policy-engine, gateway encryption; observability; VPC endpoints; policies; and online evaluation.
- **[AWS Agent Registry](docs/SECURITY_CHECKS.md#aws-agent-registry-security-checks-9)** (9 always-on core checks) - Covers Registry IAM access, publication approval, discovery authorization, encryption, organization auto-detection, record lifecycle, and provenance.
...
**249 checks across seven areas.** The assessment covers Amazon Bedrock, Amazon SageMaker AI, Amazon Bedrock AgentCore, AWS Agent Registry, always-on Agentic AI Security, optional Responsible AI GRC checks, and optional OWASP Top 10 for LLM checks. Other AI/ML services (Amazon Comprehend, Amazon Rekognition, Amazon Textract, and others) are not currently assessed.
...
| [Security Checks Reference](docs/SECURITY_CHECKS.md) | Complete reference for all 249 security checks with severity levels |
...
| [AWS AI Security Framework (AISF) Checks](docs/SECURITY_CHECKS_AISF.md) | Complete AISF-00..08 reference: the AISF control each row reports on, the shipped checks each is derived from, severity and status semantics, and why these rows are excluded from the 249-check total |
